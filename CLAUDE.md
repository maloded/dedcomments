...
### Post-Step-20 fix — root comment text/attachment shown directly in the
table; Expand narrowed to only reveal replies (done)

Reported gap against the original brief: its reference screenshot shows a
root comment's own text (and attachment, if any) visible immediately, not
gated behind a click. Before this fix, `RootCommentsTable` only fetched/
showed Username/Email/Date/`repliesCount` per row — a row's own text and
attachment were only reachable by clicking "Expand", which conflated two
different things (revealing *this comment's own content* vs. revealing its
*replies*) into one action.

**Backend — a real, small gap, not assumed away**: checked `schema.gql`
before writing any frontend code, per the task's own instruction not to
assume a field exists just because a sibling type has it. `CommentModel`
(what `rootComments.items` returns) had `text` already, but **no
`attachment` field at all** — only `ThreadCommentModel` (the
`commentThread` shape) carried one. Added it, mirroring the existing
`ThreadCommentModel.attachment` pattern exactly:
- `CommentModel.attachment: AttachmentModel` (nullable) —
  `modules/comments/models/comment.model.ts`.
- `CommentsService.getRootComments`'s `findMany` now `include`s
  `attachment: true`; `hideComment`'s `update` does too (both return a
  `CommentModel` via `toModel`).
- `createComment`: the attachment is linked to the comment via
  `tx.attachment.update` *after* the comment row is created (it can't
  exist before), so the transaction now returns `{ ...created, attachment
  }` using that same update call's result, rather than needing a second
  query.
- New `CommentsService.attachmentToModel` (private static), same shape as
  `AttachmentsService`'s own (not reused directly — no existing dependency
  from `comments` → `attachments`' full service, and this mapping is three
  lines).
- `schema.gql` regenerated by briefly booting the app locally (`PORT=4099
  npm run start:dev`, same one-off technique as Step 8's `attachment` query
  addition — `autoSchemaFile` writes at Nest bootstrap, not `tsc` build
  time). The EADDRINUSE from the still-running Docker backend on :4000 is
  harmless — the schema file is written during module registration, before
  the HTTP listen call that actually fails.
- Tests: **+3 unit** (`getRootComments` includes each item's attachment or
  null and asserts `include.attachment === true`; `createComment` links an
  attachment and returns it on the model; a `gateway.spec.ts` fixture
  needed `attachment: null` added to satisfy the now-stricter `CommentModel`
  type) — **88 unit / 56 e2e**, both green.

**Frontend**
- `rootComments.graphql`: `items` now also selects `attachment { id type
  url originalName }` — a lightweight per-row addition (no `size`/
  `processedAt`, `AttachmentPreview` doesn't need them), not the whole
  subtree. Codegen regenerated.
- `RootCommentsTable.tsx` (`RootCommentRow`): a new, **always-rendered**
  `.contentRow` under every meta row — the sanitized text (via
  `previewCommentHtml`, the same allowlist renderer `CommentThreadNode`
  already uses for read-only display, applied here for the same reason:
  consistent safe rendering, not a second implementation) and, if present,
  the attachment via the existing `AttachmentPreview` (reused as-is — its
  140×105 thumbnail cap already reads as "small" inline in a table row;
  no separate compact variant was needed). Image thumbnails open the
  existing `Lightbox`, exactly like the thread view.
- **Expand decoupled from content, not just visually — structurally**: the
  Expand/Collapse button (and the `<CommentThread>` row it toggles) is now
  conditional on `item.repliesCount > 0` — hidden entirely at 0, per the
  task's own explicit choice among "disabled/hidden". The content row has
  no dependency on `expanded` at all; it renders unconditionally from the
  `rootComments` fetch alone.
- **A real UX rough edge found and fixed while verifying, not left as
  shipped**: once a root's own text is always visible in its row, clicking
  Expand made the thread panel display that *same* text a second time —
  `CommentThreadNode` has always rendered its depth-0 node's own body
  (Step 7's original, still-correct-until-now reasoning: "the only place a
  reader ever sees a root comment's actual text"). That assumption broke
  the moment the table row itself started showing it. Fixed with a new
  `hideOwnContent` prop on `CommentThreadNode`, passed as `true` only for
  the *true* (non-re-rooted) root — `CommentThread`'s `rerootStack.length
  === 0` — suppressing just the `.text`/`.attachment` blocks. Meta (avatar/
  username/date — the connector graphic's anchor point) and actions
  (Reply/Hide/Ban/collapse-toggle) still render unconditionally: expanding
  the thread remains the only place to act *on* the root comment itself,
  and a "Continue this thread" re-rooted view (whose content was never
  shown anywhere else) still renders its own view-root's content normally.
- Text truncation (judgment call, not specified): a plain length heuristic
  (`text.length > 280`) triggers a 3-line CSS `line-clamp` with a "Show
  more"/"Show less" toggle, local `useState` per row — no `ResizeObserver`
  or layout measurement, proportionate to what "don't let one long comment
  break the row rhythm" needs.
- Mobile (375px stacked layout): the meta row and content row are now
  visually grouped as one pair (`tbody tr.contentRow { padding: var(--space-1)
  0 var(--space-3) }` vs. the meta row's `padding: var(--space-3) 0 0`) so
  a comment reads as one block, with the usual gap only *between* different
  comments — previously both rows would have gotten the same padding,
  visually splitting one comment into two evenly-spaced chunks. Also fixed
  a latent empty-space artifact: `.expandCell`'s `margin-top` was
  unconditional, leaving a small empty gap on every 0-reply row (no button
  ever rendered inside); scoped to `.expandCell:has(button)`.

**Incident during verification — flagged immediately, not glossed over**:
running the backend e2e suite locally (`npm run test:e2e`, stopping the
Docker `backend` container first, per this project's own established
convention) truncates `comments`/`authors`/`attachments`/`moderators` —
documented behavior (see the "full e2e isolation" entry above), but not
previously exercised against a Postgres instance holding ~70 comments of
real accumulated dev/demo data. Local `npm run test:e2e` and the
docker-compose dev stack's Postgres are **the same container**, reached via
`localhost:5432` vs. `postgres:5432` respectively — running e2e wiped the
entire dev dataset, including the hand-built "Aurora" connector-line test
thread (Step 13 onward) and the Step 12 pagination-seed dataset that had
been deliberately left in place for the demo video. Surfaced to the user
immediately via `AskUserQuestion` rather than silently regenerating
something different in its place; **user chose to leave it empty** —
demo data curation was already an explicitly open pre-submission task
(Steps 12/19), not something this fix was scoped to restore. Moderator was
reseeded (`npm run seed:moderator`) since nothing depends on preserving
that dev-only account either. Two minimal comments (one image attachment,
one text-file attachment) were posted afterward purely to produce this
entry's own verification screenshots — not a dataset restoration.

**Verified manually** (full `docker compose up -d --build` stack — both
`backend` and `frontend` images rebuilt for the `attachment` field/codegen
change, desktop 1280px + 375px mobile): a root comment's text renders
immediately on page load, no click; an image attachment renders as an
inline thumbnail whose click opens the `Lightbox` (confirmed both from a
fresh `rootComments` fetch **and** from Step 17's targeted live-insert
cache path — the socket-driven `handleRootCommentCreated` needed its own
`CommentCreatedPayload`/cache-write update to carry `attachment` through,
otherwise a live-inserted row would have been missing it despite a full
page reload showing it correctly); a text-file attachment renders as the
existing download chip; clicking Expand on a comment with replies reveals
only the reply tree (confirmed the root's own text no longer repeats
inside the expanded panel); Collapse hides the thread while the content
row stays; comments with `repliesCount: 0` show no Expand button at all,
their content still visible; Prev/Next and every sort column still work
correctly (re-verified against the fresh, if smaller, post-incident
dataset — page indicator, row counts, and case-insensitive username order
all correct); zero console errors throughout. `tsc --noEmit` / `next build`
/ `eslint` clean on the frontend (same one pre-existing React Compiler
info-warning on `CommentForm`'s `watch()` seen in every prior session);
backend `build`/`lint`/**88 unit**/**56 e2e** all green.

**Pending**: unchanged from before this fix, plus the dev dataset is now
much smaller than before (2 comments, from this fix's own verification) —
demo data curation (already open), the demo video, and — new, as a direct
consequence of this incident — a note for whoever curates the demo
dataset next: **do not run `npm run test:e2e` locally against a dev
